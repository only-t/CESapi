...
What is CESapi?
    ...
    uniform variables and texture or shader samplers. This tool is fully client sided and must always be loaded BEFORE every mod using it.
    If you're planning on using this API make sure your mods priority is LOWER then this APIs (current priority = 999999999).

How to use CESapi?
    While CESapi does most of the heavy lifting when it comes to managing the shaders, SOME knowledge about how shaders are created is required
    and it's recommended to have MEDIUM knowledge about GLSL ES 1.0 (the shading language used by DST) and the PostProcessor.
    Check the examplemod for a detailed explanation of how to use CESapi and examples of shaders that can be created using it.
...
Why you might need CESapi?
    ...
    Don't Starve Together uses shaders for things such as: insanity distortions, bloom or the erosion effect for when entities die to name a few.
    ...
    a more complex shader) they lack the ability to differentiate the "background" from the entities present in the scenemaking them useful
    ...
    ! For examples of what CESapi can do check the examplemod provided with this mod !
...
How does CESapi work?
    ...
    index 1 would have { 0.02, 0.01, 0.01 } and so on. The maximum amount of custom entity shaders that can be present in a single game
    is 970299 (99 * 99 * 99) (realistically impossible to reach). The maximum color value for a shader is { 0.99, 0.99, 0.99 }.
    ! THE API DOES NOT GUARANTEE THAT THE SAME SHADERS WILL HAVE THE SAME COLOR INDEX BETWEEN PLAY SESSIONS !
    ...
    has them enabled or not. However this is not a problem as the tool automatically recompiles masking shaders during server launch, filling in
    ...
    entitymask_base.ksh and postprocess_entitymasked_base.ksh. The first one is applied as a bloom shader and generates a color mask for the
    ...
    Because this API uses the BloomSampler it is required that the user has bloom disabled in their settings. CESapi creates it's own
    version of bloom, using the same method as every other custom entity shader it creates, using the color index of { 1.0, 1.0, 1.0 }.
    ! THIS COLOR INDEX IS RESERVED FOR BLOOM AND BLOOM ONLY !
...
Known issues:
    - Entity shader scale does not change based on the distance from the camera view to the entity               (High priority, potentially fixable)
    - Existing game code, specifically regarding running SetBloomEffectHandle("shaders/anim.ksh") does not work  (High priority, fixable)
    - Entity shader outlining does not change scale based on the distance from the camera view to the entity,
      creating an effect of the shader not fully covering the affected entity                                    (Low priority, potentially fixable)
    ...
    - Entity shaders do not replace the original entity causing any positional modification to create a
      duplicate                                                                                                  (Low priority, potentially unfixable)
